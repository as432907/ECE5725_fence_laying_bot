...
def estimate_distance(encoder_ticks):
    # Constants
    WHEEL_DIAMETER = 0.069  # meters (adjust to your wheel size)
    TICKS_PER_REVOLUTION = 5  # adjust based on your encoder

    # Calculate distance
    wheel_circumference = math.pi * WHEEL_DIAMETER
    revolutions = encoder_ticks / TICKS_PER_REVOLUTION
    distance = revolutions * wheel_circumference
    return distance


def move_robot(timeout, distance_threshold, drop_interval):
    global prev_error, integral, servo_index, rotation
    start_time = time.time()
    total_distance = 0
    last_drop_distance = 0
    encoder_ticks = 0

    while time.time() - start_time < timeout and total_distance < distance_threshold:
        ...
        if GPIO.input(ENC_CENTER) == GPIO.HIGH and GPIO.input(ENC_RIGHT) == GPIO.LOW and GPIO.input(ENC_LEFT) == GPIO.LOW:
            ...
            prev_error = error
            encoder_ticks += 1  # Increment ticks when moving forward
            ...
            GPIO.output(BI_2_pin, GPIO.LOW)
            prev_error = error
            encoder_ticks += 0.5  # Assume half forward motion when turning
            ...
            encoder_ticks -= 1  # Decrement ticks when moving backward

        # Update total distance
        total_distance = estimate_distance(encoder_ticks)

        # Drop cone logic based on distance
        if total_distance - last_drop_distance >= drop_interval:
            last_drop_distance = total_distance
...
def estimate_distance(encoder_ticks):
    # Constants
    WHEEL_DIAMETER = 0.069  # meters (adjust to your wheel size)
    TICKS_PER_REVOLUTION =  40 # adjust based on your encoder
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if btn_start_rect.collidepoint(x, y):
                ...
                motor_running = True
                move_robot(timeout=300, distance_threshold=5, drop_interval=0.1)
