...
def move_robot(timeout, drop_interval):
    ...
    while time.time() - start_time < timeout:  # Robot moves until timeout or manual quit
        if GPIO.input(ENC_CENTER) == GPIO.HIGH:
            pwm_driver1.ChangeDutyCycle(50)
            pwm_driver2.ChangeDutyCycle(50)
        else:
            error = 1 if GPIO.input(ENC_RIGHT) == GPIO.HIGH else (-1 if GPIO.input(ENC_LEFT) == GPIO.HIGH else 0)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT or GPIO.input(GPIOBtns[1]) == GPIO.LOW:  # Example: GPIOBtns[1] for quit
                return False
