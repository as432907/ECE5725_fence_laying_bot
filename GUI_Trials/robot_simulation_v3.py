...
def draw_disk(surface, angle):
    """
    Draws a disk where the tiny circles rotate around the large circle.
    """
    # Parameters
    ...
    # Draw the rotating tiny circles
    for i in range(7):
        # Calculate the position of each tiny circle based on the rotation angle
        theta = math.radians(i * 45 + angle)  # Add the rotation angle to each position
